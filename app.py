...
def parse_summary(summary_text):
    ...
    patterns = {
        'nome_cliente': r"1\.\s*Nome do Cliente:\s*([\w\s]+)\s*(?=\d|$)",          # Captura texto alfanumérico e espaços
        'numero_pedido': r"2\.\s*Número do Pedido:\s*([\w\d\s]+)\s*(?=\d|$)",       # Captura texto alfanumérico e espaços
        'canal_compra': r"3\.\s*Canal de Compra:\s*([\w\s]+)\s*(?=\d|$)",           # Captura texto alfanumérico e espaços
        'problema_relatado': r"4\.\s*Problema Relatado:\s*([\w\s]+)\s*(?=\d|$)",    # Captura texto alfanumérico e espaços
        'resumo_atendimento': r"Resumo do atendimento:\s*(.*)"
    }
    
    # Itera sobre os padrões e atualiza o dicionário com os valores correspondentes
    for key, pattern in patterns.items():
        match = re.search(pattern, summary_text, re.IGNORECASE)
        if match:
            summary_data[key] = match.group(1).strip()  # Extrai e remove espaços

    # Log para verificar o conteúdo extraído
...
def answer_conversation(incoming_message, conversation_history):
    ...
    data = {
        "model": "gpt-4o",
        "messages": [
            {"role": "system", "content": training},  # Instruções para a IA
        ] + conversation_history  # Envia o histórico completo
    }
...
def get_summary_within_hour():
    ...
    for sender, history in conversation_histories.items():
        ...
        if filtered_history:
            ...
            # Usa a função parse_summary para converter o texto de "resumo" em um dicionário estruturado
            summary = parse_summary(summary_text)
            
            # Log para verificar o resumo final que será passado para o front-end
